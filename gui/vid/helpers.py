...
def rotate_and_save(figure, axis, filename, frames=360, fps=30, save=False):
    def init():
        return figure,
    ...
    # Save
    if save:
        anim.save(filename, fps=fps, extra_args=['-vcodec', 'libx264'], dpi=300)


def time_animate(data, figure, ax, frames=360, fps=30, rotate_data=True, rotate_animation=False):
    frame_data = data[:, :, 0]
    if rotate_data:
